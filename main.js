...
const addToBasket = (e) => {
  ...
  const key = currentProducts.findIndex((product) => product.id
  === selectedId);
 ...
 basketTotal > 0 ? basketClearBtn.classList.add("active"):
...
categoriesButtons.forEach(btn => btn.addEventListener("click",(e) => {
  ...
  categoriesButtons.forEach((btn) => btn.classList.remove
  ("active"));
...
foundProducts.length == 0 ? emptyState.classList.add
('active') : emptyState.classList.remove('active');
